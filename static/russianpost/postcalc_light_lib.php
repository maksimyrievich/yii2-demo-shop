...
require_once __DIR__ .'/postcalc_light_config.php';
...
{
    // === НАСТРОЙКИ ЗАПРОСА ===
    $config_st = 'd927138x.beget.tech';      // Название сайта, без http: и слэшей
    $config_ml = 'kitaigorodskiy@mail.ru';   // Контактный email. Самый принципиальный параметр
    $config_ib = 'f';                        // База страховки - полная f или частичная p
    $config_r = 0.01;                        // Округление вверх поля ОценкаПолная при ответе по API. 0.01 - округление до копеек, 1 - до рублей
    $config_pr = 0;                          // Наценка в рублях за обработку заказа
    $config_pk = 0;                          // Наценка в рублях за упаковку одного отправления
    $config_cs = 'utf-8';                    // Кодировка - utf-8 или windows-1251
    $config_d  ='now';                       // Дата - в формате, который понимает strtotime(), например, '+7days','10.10.2014'
    $config_city_as_pindex = 1;              // Если 1, то в запрос вместо нас.пункта будет подставлен его почтовый индекс по умолчанию.
                                             // Настоятельно рекомендуется оставить city_as_pindex равным 1!
    // === ЖУРНАЛ СОЕДИНЕНИЙ И СТАТИСТИКА ===
    $config_log = 1;                    // Если 1, ведет в cache_dir журнал соединений вида postcalc_light_YYYY-MM.log
    ...
    $config_error_log = 1;              // Если 1, ведет в cache_dir журнал ошибок соединения вида postcalc_light_error_YYYY-MM.log
    $config_error_log_send = 10;        // Число ошибок соединения, после которых по адресу ml отсылается извещение об ошибках.
                                        // Если 0, извещение не отсылается.

    // === НАСТРОЙКИ СОЕДИНЕНИЯ ===
    $config_cache_dir = sys_get_temp_dir();     // Каталог кэширования - любой, в который веб-сервер имеет право записи
    $config_cache_valid = 600;                  // Время в секундах, после которого кэш считается устаревшим
    $config_timeout = 5;                        // Время ожидания ответа сервера в секундах
     //Список серверов для беплатной версии
    $config_servers = array('test.postcalc.ru'); //'api.postcalc.ru', - на боевом сервере
...
function postcalc_arr_from_txt($src_txt, $search = '', $limit = 0){
    ...
    // === ИСТОЧНИК ДАННЫХ ===
    $config_source = 'mysql';               // Источник - либо txt, либо mysql. Для mysql требуется поддержка MySQLi
    $config_txt_dir = __DIR__;               // Для txt: размещение текстовых файлов данных, по умолчанию - каталог postcalc_light_lib
    $config_mysql_host = 'localhost';        // Для mysql: имя хоста
    $config_mysql_user = 'd927138x_shop';    // Для mysql: имя пользователя БД
    $config_mysql_pass = 'zTYo74GQ';         // Для mysql: пароль пользователя БД
    $config_mysql_db   = 'd927138x_shop';    // Для mysql: имя базы данных
