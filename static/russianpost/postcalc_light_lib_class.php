...
{
    // === ИСТОЧНИК ДАННЫХ ===
    private $config_source = 'mysql';                // Источник - либо txt, либо mysql. Для mysql требуется поддержка MySQLi
    private $config_txt_dir = __DIR__;               // Для txt: размещение текстовых файлов данных, по умолчанию - каталог postcalc_light_lib
    private $config_mysql_host = 'localhost';        // Для mysql: имя хоста
    private $config_mysql_user = 'd927138x_shop';    // Для mysql: имя пользователя БД
    private $config_mysql_pass = 'zTYo74GQ';         // Для mysql: пароль пользователя БД
    private $config_mysql_db   = 'd927138x_shop';    // Для mysql: имя базы данных

    // === НАСТРОЙКИ ЗАПРОСА ===
    private $config_st = 'd927138x.beget.tech';     // Название сайта, без http: и слэшей
    private $config_ml = 'kitaigorodskiy@mail.ru';  // Контактный email. Самый принципиальный параметр
    private $config_ib = 'f';                       // База страховки - полная f или частичная p
    private $config_r = 0.01;                       // Округление вверх поля ОценкаПолная при ответе по API. 0.01 - округление до копеек, 1 - до рублей
    private $config_pr = 0;                         // Наценка в рублях за обработку заказа
    private $config_pk = 0;                         // Наценка в рублях за упаковку одного отправления
    private $config_cs = 'utf-8';                   // Кодировка - utf-8 или windows-1251
    private $config_d  ='now';                      // Дата - в формате, который понимает strtotime(), например, '+7days','10.10.2014'
    private $config_city_as_pindex = 1;             // Если 1, то в запрос вместо нас.пункта будет подставлен его почтовый индекс по умолчанию.
    // Настоятельно рекомендуется оставить city_as_pindex равным 1!

    // === ЖУРНАЛ СОЕДИНЕНИЙ И СТАТИСТИКА ===
    private $config_log = 1;                    // Если 1, ведет в cache_dir журнал соединений вида postcalc_light_YYYY-MM.log
    ...
    private $config_error_log = 1;              // Если 1, ведет в cache_dir журнал ошибок соединения вида postcalc_light_error_YYYY-MM.log
    private $config_error_log_send = 1;        // Число ошибок соединения, после которых по адресу ml отсылается извещение об ошибках.
    // Если 0, извещение не отсылается.

    // === НАСТРОЙКИ СОЕДИНЕНИЯ ===
    private $config_cache_dir;                    // Каталог кэширования - любой, в который веб-сервер имеет право записи
    private $config_cache_valid = 600;               // Время в секундах, после которого кэш считается устаревшим
    private $config_timeout = 5;                     // Время ожидания ответа сервера в секундах
    ...
    private function postcalc_http_request($Request, $TimeOut)
    {
    ...
    private function postcalc_arr_from_txt($src_txt, $search = '', $limit = 0)
    {
        ...
        if ($this->config_source == 'mysql') {
            $mysqli = new MySQLi($this->config_mysql_host, $this->config_mysql_user, $this->config_mysql_pass, $this->config_mysql_db);
            // Устанавливаем правильный набор символов для запроса MySQL
            if (stripos($this->config_cs, 'utf') !== false) {
                $MySQL_Charset = 'UTF8';
            } else if (stripos($this->config_cs, '1251') !== false) {
